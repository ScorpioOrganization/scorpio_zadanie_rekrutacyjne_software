    ...
    def listen_loop(self):
        ...
        while True:
            ...
            try:
                ...
                self.logerr("simulation_endpoint.TcpServer: socket timeout")
...
class SysCommands:
    ...
    def subscribe_qos(self, topic, message_name, qos):
        ...
        new_subscriber = RosSubscriber(topic, message_class, self.tcp_server, qos_profile=qos_profile)
        self.tcp_server.subscribers_table[topic] = new_subscriber
        if self.tcp_server.executor is not None:
            self.tcp_server.executor.add_node(new_subscriber)
